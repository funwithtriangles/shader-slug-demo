...
const sketchModules = import.meta.glob("./sketches/*/index.ts", {
  eager: true,
});
const sketchConfigs = import.meta.glob("./sketches/*/config.ts", {
  eager: true,
});
...
export const getSketchModuleItems = () =>
  Object.keys(sketchModules).map((path) => {
    ...
    return {
      moduleId: sketchId,
      module,
      config: processConfig(config as Record<string, unknown>, {
        fallBackTitle: sketchId,
      }),
    };
  });
